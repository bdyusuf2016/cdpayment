...
const AinDatabase: React.FC<AinDatabaseProps> = ({ clients, setClients, systemConfig }) => {
  ...
  const [confirmDelete, setConfirmDelete] = useState<{show: boolean, ain: string | null, isBulk: boolean}>({
    show: false,
    ain: null,
    isBulk: false
  });
  ...
  const handleSave = () => {
    ...
    if (editingClient) {
      setClients(prev => prev.map(c => c.ain === editingClient.ain ? newClient : c));
    } else {
    ...
    reader.onload = (event) => {
      ...
      if (newClients.length > 0) {
        setClients(prev => {
          ...
          const uniqueNewClients = newClients.filter(c => !existingAins.has(c.ain));
  ...
  const toggleSelectAll = () => {
    if (selectedAins.length === filteredClients.length && filteredClients.length > 0) {
      setSelectedAins([]);
    } else {
  ...
  const toggleSelectOne = (ain: string) => {
    setSelectedAins(prev => prev.includes(ain) ? prev.filter(a => a !== ain) : [...prev, ain]);
  };
  ...
  return (
    <div className="space-y-6">
      {/* Top Action Bar */}
      <div className={`flex flex-wrap gap-3 items-center backdrop-blur-sm p-4 rounded-2xl border shadow-sm ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-white/50 border-white'}`}>
        <button 
        ...
        {selectedAins.length > 0 && (
          <button 
            onClick={() => setConfirmDelete({ show: true, ain: null, isBulk: true })}
          ...
          >
            <i className="fas fa-trash-alt"></i> Delete Selected ({selectedAins.length})
          </button>
        )}
      ...
      {/* Table Section */}
      <div className={`rounded-[2rem] shadow-xl border overflow-hidden ${isDark ? 'bg-slate-800 border-slate-700 shadow-slate-900/50' : 'bg-white border-slate-100 shadow-slate-200/50'}`}>
        <div className={`p-6 border-b flex flex-wrap items-center justify-between gap-4 ${isDark ? 'bg-slate-900/50 border-slate-700' : 'bg-slate-50/50 border-slate-100'}`}>
          ...
          <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
             ...
             <span className="text-[10px] font-black text-blue-700 dark:text-blue-400 uppercase tracking-widest">{filteredClients.length} Profiles</span>
          </div>
        </div>
              ...
              <tr className={`${isDark ? 'bg-slate-900' : 'bg-slate-900'}`}>
                <th className="px-6 py-5 w-14 text-center">
                  <input 
                    ...
                    checked={filteredClients.length > 0 && selectedAins.length === filteredClients.length}
                    onChange={toggleSelectAll}
                  />
                </th>
                <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">AIN ID</th>
                <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Business Information</th>
                <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Communication</th>
                <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase text-right tracking-[0.2em] pr-12">Action</th>
              </tr>
            </thead>
            <tbody className={`divide-y ${isDark ? 'divide-slate-700' : 'divide-slate-50'}`}>
              {filteredClients.length === 0 ? (
                ...
                filteredClients.map((client) => (
                  <tr key={client.ain} className={`transition-all group ${selectedAins.includes(client.ain) ? 'bg-blue-50/40 dark:bg-blue-900/10' : 'hover:bg-slate-50/50 dark:hover:bg-slate-700/50'}`}>
                    ...
                    <td className="px-6 py-6">
                      <span className={`text-sm font-black px-3 py-1.5 rounded-xl border shadow-sm ${isDark ? 'bg-blue-900/30 border-blue-800 text-blue-400' : 'bg-blue-50 border-blue-100 text-blue-700'}`}>{client.ain}</span>
                    </td>
                    <td className="px-6 py-6">
                      <div className={`text-sm font-black transition-colors ${isDark ? 'text-slate-200 group-hover:text-blue-400' : 'text-slate-800 group-hover:text-blue-700'}`}>{client.name}</div>
                      <div className="text-[10px] text-slate-400 font-bold mt-1 uppercase tracking-tighter">Verified Importer/Exporter</div>
                    </td>
                    <td className="px-6 py-6">
                      {client.phone ? (
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center border ${isDark ? 'bg-green-900/20 text-green-500 border-green-800' : 'bg-green-50 text-green-500 border-green-100'}`}>
                             <i className="fab fa-whatsapp text-sm"></i>
                          </div>
                          <span className={`text-sm font-black ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{client.phone}</span>
                        </div>
                      ) : (
                        <span className="text-slate-300 dark:text-slate-500 text-[11px] italic font-bold">No WhatsApp Linked</span>
                      )}
                    </td>
                    <td className="px-6 py-6 text-right">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button 
                          onClick={() => setConfirmDelete({ show: true, ain: client.ain, isBulk: false })}
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/80 backdrop-blur-md p-4 animate-in fade-in duration-300">
          <div className={`rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-300 ${isDark ? 'bg-slate-800' : 'bg-white'}`}>
            <div className="bg-gradient-to-br from-slate-900 to-blue-900 p-10 pb-12 relative">
               ...
               <div className="flex items-center gap-5">
                  <div className="w-16 h-16 rounded-2xl bg-blue-500/20 flex items-center justify-center border border-blue-400/20 text-blue-400">
                     <i className={`fas ${editingClient ? 'fa-user-edit' : 'fa-user-plus'} text-3xl`}></i>
                  </div>
                  <div>
                    <h2 className="text-white font-black text-xl tracking-tight leading-none">{editingClient ? 'Update Profile' : 'New Registration'}</h2>
                    <p className="text-blue-300/60 text-xs font-bold mt-2 uppercase tracking-widest">Client Management System</p>
                  </div>
               </div>
            </div>
            
            <div className={`p-10 -mt-6 rounded-t-[2.5rem] space-y-6 ${isDark ? 'bg-slate-800' : 'bg-white'}`}>
              <div className="space-y-4">
                 <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">AIN Identification Number</label>
                    <input 
                      type="text" 
              ...
              <div className="flex gap-4 pt-4">
                 <button onClick={() => setShowModal(false)} className={`flex-grow py-4 rounded-2xl font-black uppercase text-[11px] tracking-widest transition-all ${isDark ? 'text-slate-400 hover:bg-slate-700' : 'text-slate-500 hover:bg-slate-100'}`}>Discard</button>
                 <button onClick={handleSave} className="flex-grow py-4 rounded-2xl font-black text-white uppercase text-[11px] tracking-widest bg-blue-600 hover:bg-blue-700 shadow-xl shadow-blue-200 transition-all active:scale-95">
      ...
      {confirmDelete.show && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/90 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className={`rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden p-10 text-center animate-in zoom-in-95 ${isDark ? 'bg-slate-800' : 'bg-white'}`}>
             ...
             <h3 className={`text-xl font-black leading-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>Confirm Deletion</h3>
             <p className={`font-medium text-sm mt-3 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
             ...
             <div className="flex flex-col gap-3 mt-8">
                <button onClick={processDelete} className="w-full py-4 bg-red-600 hover:bg-red-700 text-white font-black rounded-2xl shadow-xl shadow-red-100 transition-all active:scale-95 uppercase text-xs tracking-widest">
                  Yes, Delete Permanently
                </button>
                <button onClick={() => setConfirmDelete({ show: false, ain: null, isBulk: false })} className={`w-full py-4 font-black rounded-2xl transition-all uppercase text-xs tracking-widest ${isDark ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                  Cancel
                </button>
             </div>
          </div>
