    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className={`rounded-[2rem] border shadow-xl p-8 ${isDark ? "bg-slate-800 border-slate-700" : "bg-white border-slate-200"}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
          ...
          <div className={`rounded-2xl border p-4 ${isDark ? "bg-slate-900 border-slate-700" : "bg-rose-50 border-rose-100"}`}><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Outstanding Due</p><p className="mt-2 text-2xl font-black text-rose-500">{money(summary.due)}</p><p className="mt-1 text-[11px] font-bold text-slate-400">{dayWiseTotals.trips.toLocaleString("en-BD")} trips • {dayWiseSummary.length} day(s)</p></div>
